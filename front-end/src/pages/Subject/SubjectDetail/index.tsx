...
import { getSubject, updateSubject } from '@/services/ant-design-pro/subject';
...
const SubjectDetail: React.FC = () => {
    ...
    const onFinish = async (values: any) => {
        ...
        console.log(params);
        await updateSubject(params).then(response => {
            if (response.data) {
    ...
    const fetchAssistCalculateCateOptions = async (defaultCheckedValues: []) => {
        await listAssistCalculateCate().then(response => {
            ...
            if (defaultCheckedValues) {
                ...
                form.setFieldsValue({
                    assistCalculateCateRequired:
                        defaultCheckedValues.filter(item => item.requiredFlag)
                            .map(item => (item.assistCalculateId.toString()))
                });
            }
        }).catch(error => {
            //console.log(error);
    ...
    const fetchCurrencyConfigOptions = async (defaultCheckedValues: [], parentCheckedValues: []) => {
        const defaultCheckedStrings = defaultCheckedValues ? defaultCheckedValues.map(value => value.toString()) : [];
        const parentCheckedStrings = parentCheckedValues ? parentCheckedValues.map(value => value.toString()) : [];
        await list().then(response => {
            setAssistCurrencyConfigOptions(response.data.map(item => ({
                label: item.name,
                value: item.id.toString(),
                disabled: parentCheckedStrings.includes(item.id.toString()),
            })));
        ...
        <PageContainer>
            <Card>
                <ProForm
                    ...
                    request={async (params = {}) => {
                        ...
                        return await getSubject({ id: urlParamMap["id"], pid: urlParamMap["pid"] }).then(response => {
                            ...
                            fetchCurrencyConfigOptions(response.data.subjectCalculateConfigVo?.foreignCurrencyConfig?.currencyConfigIds, response.data.subjectCalculateConfigVo?.foreignCurrencyConfig?.parentSubjectCurrencyConfigIds);
                            ...
                            setUseCount(response.data.useCount);
                            return {
                                ...response.data,
                ...
                >
                    ...
                    <ProForm.Group>
                        <ProFormSelect width="md" name="parentName" label="上级科目" disabled>
                        </ProFormSelect >
                        <ProFormSelect width="md" name="subjectCateName" disabled label="科目类别" rules={[{ required: true }]}>
                        </ProFormSelect>
                    </ProForm.Group>
                    <ProForm.Group>
                        <ProFormRadio.Group width="md" name="balanceDirection" label="余额方向" radioType="radio"
                            disabled={subjectType === 0}
                        ...
                        <ProFormSwitch width="md" name="disable" label="科目状态" rules={[{ required: true }]} checkedChildren="启用" unCheckedChildren="停用" initialValue={true} disabled />
                        <ProFormText
                            width="md"
                            name="mnemonicCode"
                            label="助记码"
                    ...
                    <ProForm.Group>
                        <ProFormSwitch
                            ...
                            disabled={parentEnableNumberCalculateConfig || useCount}
                        />
                        <ProFormText
                            width="md"
                            ...
                            disabled={!enableNumberCalculateConfig || parentEnableNumberCalculateConfig || useCount}
                    ...
                    <ProForm.Group>
                        <ProFormSwitch
                            ...
                            disabled={parentEnableAssistCalculateConfigs || useCount}
                        />
                        <ProForm.Item style={{ marginTop: 2 }} hidden={!enableAssistCalculateConfigs} name="assistCalculateCate">
                            <CheckboxGroup options={assistCalculateCateOptions}
                                disabled={!enableAssistCalculateConfigs || parentEnableAssistCalculateConfigs || useCount}
                    ...
                    <ProForm.Group>
                        ...
                        <ProForm.Item style={{ marginTop: 2 }} hidden={!enableAssistCalculateConfigs} name="assistCalculateCateRequired">
                            <CheckboxGroup options={assistCalculateCateRequired}
                                disabled={!enableAssistCalculateConfigs || useCount} />
                        </ProForm.Item>
                    </ProForm.Group>
                    <ProForm.Group>
                        <ProFormSwitch
                            ...
                            disabled={parentEnableForeignCurrencyConfig || useCount}
                        />
                        <ProForm.Item style={{ marginTop: 2 }} name="assistCheckboxGroup" hidden={!enableForeignCurrencyConfig}>
                            <CheckboxGroup options={assistCurrencyConfigOptions} disabled={useCount} />
                        </ProForm.Item>
                    </ProForm.Group>
                    <ProForm.Group>
                        <ProFormSwitch
                            ...
                            disabled={!enableForeignCurrencyConfig || parentEnableForeignCurrencyConfig || useCount}
